...
Clazz_load (["JU.P3"], "J.adapter.smarter.XtalSymmetry", ["java.lang.Boolean", "$.Float", "java.util.Hashtable", "JU.BS", "$.M3", "$.M4", "$.P3i", "$.PT", "$.SB", "$.V3", "J.adapter.smarter.Atom", "J.api.Interface", "JU.BSUtil", "$.Logger"], function () {
...
if (acr.strSupercell != null) this.setSuperCellFromString (acr.strSupercell);
 else this.setSupercellFromPoint (acr.ptSupercell);
...
var ptx = this.setSym (JU.P3.new3 (1, 0, 0));
var pty = this.setSym (JU.P3.new3 (0, 1, 0));
var ptz = this.setSym (JU.P3.new3 (0, 0, 1));
var pt =  new JU.P3 ();
...
this.setNotionalUnitCell ([0, 0, 0, 0, 0, 0, ptx.x, ptx.y, ptx.z, pty.x, pty.y, pty.z, ptz.x, ptz.y, ptz.z], null, this.asc.getAtomSetAuxiliaryInfoValue (-1, "unitCellOffset"));
...
Clazz_defineMethod (c$, "applyAllSymmetry", 
 function (ms, bsAtoms, forceCheckSpecial) {
...
this.checkAll = (forceCheckSpecial || this.asc.atomSetCount == 1 && this.asc.checkSpecial && this.latticeOp >= 0);
...
if (bsAtoms == null) atom.setT (c);
 else if (atom.distance (c) < 0.0001) bsAtoms.set (atom.index);
 else {
...
for (var tx = this.minXYZ.x; tx < this.maxXYZ.x; tx++) for (var ty = this.minXYZ.y; ty < this.maxXYZ.y; ty++) for (var tz = this.minXYZ.z; tz < this.maxXYZ.z; tz++) {
iCell++;
if (tx != 0 || ty != 0 || tz != 0) pt = this.symmetryAddAtoms (tx, ty, tz, cell555Count, pt, iCell * operationCount, cartesians, ms, forceCheckSpecial);
}
...
Clazz_defineMethod (c$, "symmetryAddAtoms", 
 function (transX, transY, transZ, baseCount, pt, iCellOpPt, cartesians, ms, forceCheckSpecial) {
...
var pt0 = (forceCheckSpecial ? 0 : checkSpecial ? pt : checkRange111 ? baseCount : 0);
...
isRev = (def).startsWith ("!");
if (isRev) def = (def).substring (1);
var symTemp = ms.getSymTemp (true);
symTemp.setSpaceGroup (false);
var i = symTemp.addSpaceGroupOperation ("=" + def, 0);
if (i < 0) return null;
...
return pts;
}, "JM.ModelSet,J.api.SymmetryInterface,~O");
...
function (s) {
return  new JU.SB ().append ("{\t").appendI (Clazz_floatToInt (s.m00)).append ("\t").appendI (Clazz_floatToInt (s.m01)).append ("\t").appendI (Clazz_floatToInt (s.m02)).append ("\t").append (JS.SymmetryOperation.twelfthsOf (s.m03)).append ("\t}\n").append ("{\t").appendI (Clazz_floatToInt (s.m10)).append ("\t").appendI (Clazz_floatToInt (s.m11)).append ("\t").appendI (Clazz_floatToInt (s.m12)).append ("\t").append (JS.SymmetryOperation.twelfthsOf (s.m13)).append ("\t}\n").append ("{\t").appendI (Clazz_floatToInt (s.m20)).append ("\t").appendI (Clazz_floatToInt (s.m21)).append ("\t").appendI (Clazz_floatToInt (s.m22)).append ("\t").append (JS.SymmetryOperation.twelfthsOf (s.m23)).append ("\t}\n").append ("{\t0\t0\t0\t1\t}\n").toString ();
}, "JU.M4");
c$.dumpCanonicalSeitz = Clazz_defineMethod (c$, "dumpCanonicalSeitz", 
function (s) {
return  new JU.SB ().append ("{\t").appendI (Clazz_floatToInt (s.m00)).append ("\t").appendI (Clazz_floatToInt (s.m01)).append ("\t").appendI (Clazz_floatToInt (s.m02)).append ("\t").append (JS.SymmetryOperation.twelfthsOf ((s.m03 + 12) % 12)).append ("\t}\n").append ("{\t").appendI (Clazz_floatToInt (s.m10)).append ("\t").appendI (Clazz_floatToInt (s.m11)).append ("\t").appendI (Clazz_floatToInt (s.m12)).append ("\t").append (JS.SymmetryOperation.twelfthsOf ((s.m13 + 12) % 12)).append ("\t}\n").append ("{\t").appendI (Clazz_floatToInt (s.m20)).append ("\t").appendI (Clazz_floatToInt (s.m21)).append ("\t").appendI (Clazz_floatToInt (s.m22)).append ("\t").append (JS.SymmetryOperation.twelfthsOf ((s.m23 + 12) % 12)).append ("\t}\n").append ("{\t0\t0\t0\t1\t}\n").toString ();
}, "JU.M4");
...
ptemp.setT (ipt);
uc.toFractional (ptemp, false);
info1 = "inversion center|" + this.strCoord (ptemp);
} else if (isRotation) {
if (haveInversion) {
...
m2.m13 += vtrans.y;
m2.m23 += vtrans.z;
}xyzNew = (this.isBio ? m2.toString () : JS.SymmetryOperation.getXYZFromMatrix (m2, false, false, false));
...
this.centering = null;
}if (!isFinal) this.centering.scale (0.083333336);
